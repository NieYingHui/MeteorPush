...
# ✅ 成功输出：Formatting /tmp/kraft-combined-logs with metadata.version 3.7-IV4.

# 启动 Kafka
bin/kafka-server-start.sh -daemon config/kraft/server-local.properties
# 使用lsof -i :9092查看是否运行成功
```
...
./comet/comet_server --config ../conf/comet.conf # 如果配置了代理：env -u http_proxy -u https_proxy -u ALL_PROXY \ ./comet/comet_server --config conf/comet.conf &
